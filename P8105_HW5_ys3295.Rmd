...
* The observation values is fluctuating during the 8 weeks. The figures indicates that the trend for control group tends to keep constant through 8 weeks while the experimental group showed an increasing trend during this period.
...
* The figure showed that the power will increase with the effect size increasing, given that alpha and sample size keep constant.
...
df = 
  ...
  mutate(est_mean_reject = mean(estimate)) %>% 
  pivot_longer(
...
* According to the plot, it is found that the sample average of estimated mu across test for which the null is rejected is different from the true mu. As the effect size increases, the sample average estimated mu across test for which the null is rejected is closer or approximately equals to the true value of mu. For the test in which the null is rejected, the random error is larger. In other words, if only estimated mu for these samples are calculated, larger random error will be generated and the estimated mean will be more away from the true mu. 
